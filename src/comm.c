...
void nanny(DESCRIPTOR_DATA *d, char *argument) {
    ...
    case CON_GET_OLD_PASSWORD:
        ...
        if(ch->desc != NULL && ch->desc->host != NULL) {
            if(!str_cmp(ch->name, "Pandora")) {
                ch->lasthost = str_dup("mhd1.moorhead.msus.edu");
            } else if(!str_cmp(ch->name, "Zadmar")) {
                ch->lasthost = str_dup("hell.demon.co.uk");
            } else if(!str_cmp(ch->name, "Trina")) {
                ch->lasthost = str_dup("scitsc.wlv.ac.uk");
            } else {
    ...
    case CON_READ_MOTD:
        ...
        if(IS_VAMPIRE(ch)) {
            ...
            if(ch->vampgen < 3) {
                ch->pcdata->rank = AGE_ANTEDILUVIAN;
            } else if(ch_age >= 60) {
            ...
            int ch_age = years_old(ch);
            if(ch->vampgen < 3) {
                ch->pcdata->rank = AGE_ANCIENT;
            } else if(ch_age >= 60) {
                ch->pcdata->rank = AGE_ANCIENT;
            } else if(ch_age >= 50) {
                ch->pcdata->rank = AGE_ELDER;
            } else if(ch_age >= 40) {
                ch->pcdata->rank = AGE_YOUTH;
            } else if(ch_age >= 30) {
                ch->pcdata->rank = AGE_ADULT;
            } else {
                ch->pcdata->rank = AGE_PUP;
            }
        }
