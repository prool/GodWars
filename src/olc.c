...
char *get_exit_flag_string(int exit) {
    ...
    if(exit & EX_BASHPROOF) {
        strcat(buf, " bashproof");
    }
    if(exit & EX_PICKPROOF) {
        strcat(buf, " pickproof");
    }
    if(exit & EX_PASSPROOF) {
        strcat(buf, " passproof");
    }
